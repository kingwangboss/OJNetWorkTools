...
NSString *const kkXMLReaderTextNodeKey		= @"text";
NSString *const kkXMLReaderAttributePrefix	= @"@";


@interface XMLReaders ()
...
@implementation XMLReaders

#pragma mark - Public methods

+ (NSDictionary *)dictionaryForXMLData:(NSData *)data error:(NSError **)error
{
    XMLReaders *reader = [[XMLReaders alloc] initWithError:error];
...
+ (NSDictionary *)dictionaryForXMLString:(NSString *)string error:(NSError **)error
{
    ...
    return [XMLReaders dictionaryForXMLData:data error:error];
}
...
{
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    return [XMLReaders dictionaryForXMLData:data options:options error:error];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([self.textInProgress length] > 0)
    {
        ...
        [dictInProgress setObject:[trimmedString mutableCopy] forKey:kkXMLReaderTextNodeKey];
